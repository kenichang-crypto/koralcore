...
import '../../../warning/presentation/pages/warning_page.dart';
...
class _HomePageView extends StatelessWidget {
        ...
        child: SafeArea(
          child: Column(
            children: [
              ...
              // 設備列表區域
              Expanded(
                child: controller.selectionType == SinkSelectionType.allSinks
                    ? _buildAllSinksView(context, controller, l10n)
                    : devices.isEmpty
                        ? _EmptyState(l10n: l10n, isAllSinksMode: false)
  ...
  Widget _buildAllSinksView(BuildContext context, HomeController controller, AppLocalizations l10n) {
    final sinks = controller.sinks;
    final allDevices = controller.filteredDevices;
    final appContext = Provider.of<AppContext>(context, listen: false);

    // Group devices by sink asynchronously
    return FutureBuilder<Map<String, List<DeviceSnapshot>>>(
      future: _groupDevicesBySink(allDevices, appContext),
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          return const Center(child: CircularProgressIndicator());
        }

        final devicesBySink = snapshot.data!;
        ...
        if (sinksWithDevices.isEmpty) {
          return _EmptyState(l10n: l10n, isAllSinksMode: true);
        }
  ...
  Future<Map<String, List<DeviceSnapshot>>> _groupDevicesBySink(
    ...
    for (final device in devices) {
      final deviceMap = await appContext.deviceRepository.getDevice(device.id);
      if (deviceMap != null) {
...
class _TopButtonBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // But they still exist in layout and affect spacing
    // btn_add_sink: 56×44dp, padding 16dp (start/end), 10dp (top/bottom)
    // btn_warning: 56×44dp, aligned with btn_add_sink
    // Even though visibility="gone", sp_sink_type's marginTop is relative to btn_add_sink's bottom
    // So we need to reserve space for proper spacing
    return const SizedBox(
      height: 44.0, // dp_44 (btn_add_sink height)
    );
  }
}
...
class _SinkSelectorBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Note: marginTop 10dp is relative to btn_add_sink's bottom (which is 44dp height)
    return Padding(
      padding: EdgeInsets.only(
...
class _EmptyState extends StatelessWidget {
  final AppLocalizations l10n;
  final bool isAllSinksMode;

  const _EmptyState({
    required this.l10n,
    this.isAllSinksMode = false,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return EmptyStateWidget(
      title: isAllSinksMode ? l10n.deviceInSinkEmptyTitle : l10n.deviceEmptyTitle,
      subtitle: isAllSinksMode ? l10n.deviceInSinkEmptyContent : l10n.deviceEmptySubtitle,
    );
  }
}
...
class _HomeDeviceGridTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool isFavorite = device.favorite; // PARITY: data.favorite in reef-b-app DeviceAdapter.bind()
    final String? deviceSinkId = device.sinkId; // PARITY: data.sinkId in reef-b-app
    final String? deviceGroup = device.group; // PARITY: data.group in reef-b-app (LedGroup enum)
